...
void tmPlannerNode::loadParameters() {
  ...
  CHECK(nh_.getParam("sensor_type", sensor_type) &&
        nh_.getParam("measurement_frequency", measurement_frequency) &&
  ...
  CHECK(nh_.getParam("optimization_method", optimization_method) &&
        ...
        nh_.getParam("cmaes_objective",cmaes_objective) &&
  ...
  optimization_parameters.cmaes_objective = cmaes_objective;
...
void tmPlannerNode::planningTimerCallback(const ros::TimerEvent&) {
  ...
  if (time_along_trajectory > tmplanner_.getTrajectory().getMaxTime()) {
    ...
    logger1.writeLineToFile("time_trace_traj",tot_trajectory_time,tmplanner_.getGridMap().computeCovarianceTrace());
    if(optimization_parameters_.cmaes_objective == "covariance")
    {
      // Write the trace and total time in trajectory
      logger1.writeLineToFile("time_trace_traj",tot_trajectory_time,tmplanner_.getGridMap().computeCovarianceTrace());
    }
